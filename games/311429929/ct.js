...
const ct = {
     ...
     */
    version: '1.4.2',
    meta: [{"name":"Arcade Park","author":"Quentin Launay/Maxime Baudoin/ Clement Lafon/Jules Serrato","site":"","version":"1.0.0"}][0],
    main: {
        fpstick: 0,
        pi: 0
    },
...
setInterval(mycode, 1000);

function mycode() {
    if (ct.room.ballonCount < 5) {
        ct.room.ballonCount++;
        ct.types.copy('ballonRed', 1000, ct.random.range(128, 384));
    } else {
        setTimeout(function () {
            if (ct.room.ballonCount !== ct.room.score) { // perdu, gameover
                document.location.href='../../server.php?gameover';
            } else { // gagné, jeu suivant
                document.location.href='../../server.php?gamenext';
            }
        }, 2000);        
    };
};;
...
ct.rooms.templates['0'] = {
    name: '0',
    width: 1024,
    height: 575,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[]'),
    bgs: JSON.parse('[{"depth":-1,"texture":"background_final_ecran","extends":{"repeat":"no-repeat"}},{"depth":1,"texture":"background_final_sans_ecran","extends":{"repeat":"no-repeat"}}]'),
    ...
    onDraw() {
        let viseur = ct.types.copy('viseur', ct.mouse.x, ct.mouse.y);
viseur.kill = true ;

// let carabine = ct.types.copy('carabine', )

ct.time;
// this.scoreLabel.text = 'Score: ' + this.score;

let carabine = ct.types.copy('carabineFace', 512, 448).visible = false;
if (ct.mouse.x > 320 && ct.mouse.x < 640) {
    carabine = ct.types.copy('carabineFace', 512, 448);
} else if (ct.mouse.x < 320) {
    carabine = ct.types.copy('carabineGauche', 480, 480);
} else if (ct.mouse.x > 640) {
    carabine = ct.types.copy('carabineDroite', 544, 480);
};
carabine.kill = true;
    },
    onLeave() {
...
this.scoreLabel.x = 32;
this.scoreLabel.y = 96;
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        soundsError: 0,
        sounds: {},
        registry: [{"ballon red":{"frames":1,"shape":{"type":"circle","r":26},"anchor":{"x":0.49621212121212116,"y":0.17018779342723003}},"background_final_sans_ecran":{"atlas":"./img/t0.png","frames":0,"shape":{"type":"strip","points":[{"x":486.75,"y":-199.60714285714286},{"x":484.64285714285717,"y":133.75},{"x":-488.3928571428571,"y":132.82142857142856},{"x":-488.17857142857144,"y":-197.42857142857144}],"closedStrip":true},"anchor":{"x":0.5,"y":0.5}},"background_final_ecran":{"atlas":"./img/t1.png","frames":0,"shape":{"type":"rect","top":192,"bottom":128,"left":480,"right":480},"anchor":{"x":0.5,"y":0.5}},"crosshair_rouge_(1)":{"frames":1,"shape":{"type":"circle","r":16},"anchor":{"x":0.49122807017543857,"y":0.49122807017543857}},"IMG_0040":{"frames":1,"shape":{"type":"rect","top":256,"bottom":256,"left":15,"right":15},"anchor":{"x":0.5,"y":0.5}},"gauche":{"frames":1,"shape":{"type":"rect","top":110,"bottom":110,"left":110,"right":110},"anchor":{"x":0.3931818181818182,"y":0.7378787878787879}},"droite":{"frames":1,"shape":{"type":"rect","top":110,"bottom":110,"left":110,"right":110},"anchor":{"x":0.14602272727272728,"y":0.7363636363636363}}}][0],
...
ct.types.templates["viseur"] = {
    depth: 5,
    texture: "crosshair_rouge_(1)",
    onStep: function () {
        this.move();
    },
    onDraw: function () {
...
ct.types.list['viseur'] = [];
ct.types.templates["ballonRed"] = {
    depth: 0,
    texture: "ballon red",
    onStep: function () {
        this.speed = 2+(difficulte/2);
this.direction = ct.random.range(157, 202);
this.move();

if (this.x < 32 || this.y > 416) {
    ...
    onCreate: function () {
        //this.direction = ct.random.range(175, 220);
    },
    extends: {}
};
ct.types.list['ballonRed'] = [];
ct.types.templates["carabineFace"] = {
    depth: 6,
    texture: "IMG_0040",
    onStep: function () {
        this.move();
...
ct.types.list['carabineFace'] = [];
ct.types.templates["carabineGauche"] = {
    depth: 6,
    texture: "gauche",
    onStep: function () {
        this.move();
...
ct.types.list['carabineGauche'] = [];
ct.types.templates["carabineDroite"] = {
    depth: 6,
    texture: "droite",
    onStep: function () {
        this.move();
